<template lang="html">
  <div>
    ...
    <div v-if="approximationTicksData.img !== ''" class="d-inline-block">
      <v-img
          ...
          <div>
            {{
              $t("$.approximation.processors", {
                value: Math.round(
                  calculateJson(approximationProcessorsData.json),
                ),
              })
            }}
          </div>
...
@Component({
  async asyncData(context) {
    const [resAlgorithm, resMatrix] = await Promise.all([
      ...
      axios.get(
        `/approximation/${context.params.algorithmId}/matrix.json`,
      {
        baseURL: process.env.API_ENDPOINT,
      },)
    ]);
    ...
    const [
      ...
      axios.get(`/approximation/${context.params.algorithmId}/processors.json`,
      {
        baseURL: process.env.API_ENDPOINT,
      }),
      axios.get(`/approximation/${context.params.algorithmId}/ticks.json`,
      {
        baseURL: process.env.API_ENDPOINT,
      }),
    ]);
...
export default class ApproximationPage extends Vue {
  ...
  public get breadcrumbItems() {
    return [
      ...
      },
      {
        ...
        to: `/algorithms/${this.algorithmData.algorithmId}/determinants/approximation/${
  ...
  public calculateJson(json: any) {
    ...
    if (this.argumentsData.some((e) => e.value <= 0)) return "0";
